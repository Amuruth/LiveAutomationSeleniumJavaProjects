...
public class TC_RF_014 {
	...
	@Test
	public void verifyMandetoryFieldsSymbolAndColorInRegisterAccountPage() {
		...
		WebElement fristNameLabel = driver.findElement(By.cssSelector("label[for='input-firstname']"));	
		JavascriptExecutor jse = (JavascriptExecutor)driver;
		String fnContent = (String)jse.executeScript("return window.getComputedStyle(arguments[0], '::before').getPropertyValue('content');", fristNameLabel);
		String fnColor = (String)jse.executeScript("return window.getComputedStyle(arguments[0], '::before').getPropertyValue('color');", fristNameLabel);
        ...
        WebElement passwordConfirmLabel = driver.findElement(By.cssSelector("[class='pull-right']"));	
		...
		WebElement privacyPolicyLabel = driver.findElement(By.cssSelector("label[for='input-confirm']"));	
		...
		String privacyPolicyolor = (String)jse.executeScript("return window.getComputedStyle(arguments[0], '::before').getPropertyValue('color');", privacyPolicyLabel);		
		Assert.assertEquals(privacyPolicyContent, expectedContent);
		Assert.assertEquals(privacyPolicyolor, expectedColor);
